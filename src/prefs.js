...
const SCHEMA = {
  ...
  // When true (default), Clawd spawns scripts/companion-watcher.js as a
  // background child process on startup and stops it on quit, so the games/
  // music/video/meeting companion features run automatically alongside
  // Clawd instead of needing `npm run watch-companion` started by hand.
  companionWatcherAutoStart: { type: "boolean", default: true },
  // Configurable break-reminder threshold for companion-watcher.js (minutes
  // of continuous same-activity before a "stretch your legs?" nudge). Read
  // by main.js's Settings > Companion tab handler, written out to
  // scripts/companion-settings.json for the standalone watcher process to
  // pick up (it re-reads that file every poll, no restart needed).
  companionBreakReminderMinutes: { type: "number", default: 90 },
  // Off by default: reusing the Telegram approval bot for companion
  // streak/break/achievement pings is extra chatter beyond what the user
  // explicitly configured Telegram for, so it's an opt-in on top of an
  // already-working Telegram approval setup (see companion-telegram-alerts.js).
  companionTelegramAlertsEnabled: { type: "boolean", default: false },
  // Cosmetic, level-gated idle-pose accessory (see ACCESSORY_UNLOCKS in
  // scripts/companion-watcher.js and applyCompanionAccessory in main.js).
  // "none" is always allowed; other values only take visible effect on
  // themes that ship a matching idle-accessory-*.svg file (currently just
  // Cozy Cat) and once the companion level requirement is met -- picking an
  // accessory you haven't unlocked yet, or one the active theme doesn't
  // support, is a silent no-op rather than an error.
  companionEquippedAccessory: { type: "string", default: "none" },
  // Phone companion sync (Android app mirrors the desktop pet live over a
  // Cloudflare Worker relay, see cloud/README.md). Off by default -- opt-in
  // once the user deploys their own relay and generates a pairing code.
  // cloudRelayUrl is the user's own Worker URL (e.g.
  // https://ilia-companion-sync.<subdomain>.workers.dev); cloudSyncCode is a
  // random code generated by "Generate Pairing Code" in Settings > Companion
  // > Phone Sync, entered once in the Android app to link it to this
  // desktop. Both are mirrored to scripts/companion-settings.json for the
  // standalone watcher process (see companion-cloud-sync.js).
  cloudSyncEnabled: { type: "boolean", default: false },
  cloudRelayUrl: { type: "string", default: "" },
  cloudSyncCode: { type: "string", default: "" },
